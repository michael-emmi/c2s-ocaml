(** Translation from Boogie programs to concurrent programs. *)

(*** ToDo this was copied from CpToBpl.ml and is under construction. *)
...
module TT = CpAst.Type
let rec type_ = function
	| T.Bool -> TT.Bool
	| T.Int -> TT.Int
	| T.T (x,_) -> TT.T x
	| T.Map (_,ts,t) -> TT.Map (List.map type_ ts, type_ t)
	| t -> failwith
		  << sprintf "Illegal type `%s' for concurrent programs."
		  <| T.to_string t

module L = BplAst.Literal
module LL = CpAst.Literal
let lit = function
	| L.True -> LL.Bool true
	| L.False -> LL.Bool false
	| L.Num n -> LL.Num n
	| L.Bv _ -> failwith "Illegal type BV for concurrent prorams."

module Bop = BplAst.BinaryOp
module BBop = CpAst.BinaryOp
let oper = function
	| Bop.Iff -> BBop.Iff
	| Bop.Imp -> BBop.Imp
	| Bop.Or -> BBop.Or
...
let rec expr e = match e with
	...
	| E.Q (q,_,xs,_,_,e) -> 
		EE.Q (quant q, List.map (fun (x,t) -> ident x, type_ t) xs, expr e)
	| e -> failwith
...
module SS = CpAst.Statement
module LLs = CpAst.LabeledStatement

let rec stmt s =
	match s with
	| ls,s -> begin
		  match List.map ident ls, s with
		  | ls, S.Goto ids -> [ LLs.S (ls, SS.Goto (List.map ident ids)) ]
		  ...
		  | ls, S.Assign (xs,es) ->
				...
				[ LLs.S (ls, SS.Assign (List.map lval xs, List.map expr es, None )) ]
